...
let getType (value: evT): types = 
	...
	| RecClosure(_) -> failwith("RecClosure value has no type")
...
let int_times (v1: evT) (v2: evT) : evT =
	match (v1, v2) with
	| (Int(x),Int(y)) -> Int(x*y)
	| (_,_) -> failwith("Type error in Times");;

let int_sub (v1: evT) (v2: evT) : evT =
	match (v1, v2) with
	| (Int(x),Int(y)) -> Int(x-y)
	| (_,_) -> failwith("Type error in Sub");;
...
let bool_equal (v1: evT) (v2: evT) : bool =
	...
	| (RecClosure(_), RecClosure(_))-> failwith("Recursive Functions can't be compared")
...
let rec eval (expression: exp) (myEnv: evT env): evT =
	...
	| Times(e1,e2) -> int_times (eval e1 myEnv) (eval e2 myEnv)
	| Sub(e1,e2) -> int_sub (eval e1 myEnv) (eval e2 myEnv)
	| Equal(e1,e2) -> Bool(bool_equal (eval e1 myEnv) (eval e2 myEnv) )
	| Not(e) -> Bool(bool_not (eval e myEnv))
	| Ifthenelse(cond,e1,e2) -> 
		let evaluatedCond = eval cond myEnv in 
			(
				match evaluatedCond with
				| Bool(true) -> eval e1 myEnv
				| Bool(false) -> eval e2 myEnv
				| _ -> failwith("If-then-else condition is not a bool expression")
			)
	| Let(id,e,eBody) -> 
		let newEnv = bind myEnv id (eval e myEnv) in 
			eval eBody newEnv
	| Letrec(fName,fArg,fBody,letBody) ->	let bodyEnv = bind myEnv fName (RecClosure(fName,fArg,fBody,myEnv))
	...
	| Map(_) -> (* evT_funApplication applica il parametro attuale actualValue alla 
					...
					(
						...
						| RecClosure(fName,fArg,fBody,functionEnv) -> (* chiusura ricorsiva *)
								(* aggiungo questa funzione nell'ambiente della funzione stessa *)
				 				let recursiveEnv = bind functionEnv fName functionClosure  in
				 					let actualEnv = bind recursiveEnv fArg actualValue in (* aggiungo il bindign del parametro attuale all'ambiente *) 
				 					eval fBody actualEnv (* passo a valutare il corpo della funzione *)
						...
						| Map(e1,e2) -> 
								(* set_mapFunction controlla che funClosure e set siano rispettivamente
								   ...
								   la funzione ad ogni elemento del set e restituisce un set 
									 contenente tutti gli elementi del set di partenza mappati *)
										...
										| RecClosure(_,_,_,_) -> 
											(
												...
												| Set(type1,valuesList) -> 
													...
													let rec aux lista acc = 
														(
															match lista with
															| [] -> acc
															| h::t -> let evaluatedValue = evT_funApplication funClosure h in 
																					(* controllo che non siano stati prodotti elementi duplicati *)
																					if (List.mem evaluatedValue acc) = true then failwith("Map function produces duplicates")
																					else aux t (evaluatedValue::acc)
														) in let mappedList = aux valuesList emptyEvtList in (* lista mappata *)
																(
																	match mappedList with (* controllo se il tipo degli elementi del set è cambiato *)
																	| [] -> Set(type1,[])
																	| h::t -> Set(getType h, List.rev mappedList)
																)
												| _ -> failwith("Second argument is not a set")
											)
										| Unbound -> failwith("Function not found")
